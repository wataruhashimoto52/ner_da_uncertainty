...
class MultiLabelTextProcessor:
            ...
            if not pd.isna(row[-1]):
                ...
                if row[-1] != "O":
                    if row[1] in [
                        ...
                        "Es",
                        "Nl",
                    ]:  # Add language prefix if they are present
                        text.append("<" + row[1] + ">")
                        label.append("O")
                    text.append("<" + row[-1] + ">")
    ...
    def get_examples(self, dsplit):
        ...
        data_df = pd.read_csv(
            os.path.join(self.data_dir, dsplit + ".txt"),
            sep=" |\t",
            header=None,
...
class DataForTrainer:
    def __init__(self, tokenizer, b_size, label_map, file_dir, mask_rate, labels):
    ...
    def create_dataset_files(self, file_dir):
        ...
        for dsplit in ["train", "dev"]:
            ...
            features = self.convert_examples_to_features(
                examples, self.tokenizer, is_train=(dsplit == "train")
            )
            ...
            masked_ids = torch.tensor(
                [f.masked_ids for f in features], dtype=torch.long
            )
            entity_mask = torch.tensor(
                [f.entity_mask for f in features], dtype=torch.long
            )

            dataset = TensorDataset(
                input_ids, input_mask, label_ids, masked_ids, entity_mask
            )
    ...
    def convert_examples_to_features(
        self, examples, tokenizer, max_seq_length=128, is_train=True
    ):
        ...
        for example in examples:
            ...
            masked_ids = [
                input_ids.copy() for i in range(30)
            ]  # Use different masking for diff epoch, [list]*20 is bug coz it refer to the same list
            ...
            for i, label in enumerate(example.label):
                ...
                if label != "O":
                    ...
                    diff_class_labels = [
                        l for l in self.labels if l not in same_class_labels
                    ]
    ...
    def label_to_token_id(self, label):
        label = "<" + label + ">"
        assert label in [f"<{l}>" for l in self.labels]
