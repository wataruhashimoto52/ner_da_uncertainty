...
class Data:
    ...
    def create_dataset_files(self):
        processor = MultiLabelTextProcessor(self.data_path)
        for filename in [self.file_name]:
            print(f"Generating dataloader for {filename}")
            examples = processor.get_examples(os.path.join(self.data_path, filename))
            ...
            entity_masked_ids = torch.tensor(
                [f.entity_masked_ids for f in features], dtype=torch.long
            )
            entity_mask = torch.tensor(
                [f.entity_mask for f in features], dtype=torch.long
            )
            o_masked_ids = torch.tensor(
                [f.o_masked_ids for f in features], dtype=torch.long
            )
        ...
        for example in examples:
            ...
            for i, label in enumerate(example.label):
                ...
                if label != "O":
                    ...
                    mask_len = trunc_gauss(
                        subword_len[i] * self.mu_ratio, self.sigma, 1, subword_len[i]
                    )
            ...
            if len(label_ids) > max_seq_length:
                ...
                entity_masked_ids.extend(
                    [0] * (max_seq_length - len(entity_masked_ids))
                )
    ...
    def label_to_token_id(self, label: str):
        label = "<" + label + ">"
